...
scripts/
  ...
  generate_wrappers.sh  → Reads wrappers.conf, generates wrapper scripts
  ...
  shell_hook.sh         → Post-command hook to clean up PATH after loading
  bin/
...
### Wrapper generation
Wrappers are generated by `generate_wrappers.sh` using heredoc templates with `BINARY_NAME` placeholder replaced via `sed`. Each wrapper:
