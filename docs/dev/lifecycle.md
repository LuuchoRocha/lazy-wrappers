...
3. **Copy scripts** — copies all files from `scripts/` (config, generate_wrappers, nvmload, rbenvload, shell_hook, wrappers.conf) to the install directory.
...
- `shell-hook` is sourced to install the post-command cleanup function.
...
2. The wrapper script starts in a subshell:
   - Sets a recursion guard env var (`_LW_LOADING_node=1`).
...
1. Cleans shell RC files — removes all lines referencing `lazy-wrappers`, the wrapper directories, or `shell-hook`.
...
Wrapper intercepts → loads nvm → exec real node → sets flag
    │
    ▼
shell-hook fires → removes wrappers from PATH → loads nvm into parent
    │
    ▼
